...
if (cameraView != "GUNNER" && {(currentWeapon player != "") && {isNull objectParent player}}) then { //isNull objectParent player = checks if player is in vehicle
    private _icon = GVAR(fireControl_crosshairIcon);
    ...
    private _vectorStart = player modelToWorldWorld (player selectionPosition ["rightHand", "Memory"]);
    private _vectorEnd = (_vectorStart vectorAdd (player weaponDirection currentWeapon player vectorMultiply _vectorMaxCheck));
    ...
    _vectorEnd = ASLToAGL (_vectorStart vectorAdd (player weaponDirection currentWeapon player vectorMultiply _distanceToTarget));
    } else {
    _distanceToTarget = (player distance cursorObject) min GVAR(fireControl_crosshairMaxRange);
    _vectorEnd = ASLToAGL (_vectorStart vectorAdd (player weaponDirection currentWeapon player vectorMultiply _distanceToTarget));
    };
